...
void adr_zp(){
	...
	cp_register(pcl, abrl);
	cp_register(pch, abrh);
	// load adr low from memory
	set_rw2read();
	access_memory();
	// write adr low and zp to adr bus
...
void adr_izx(){
	...
	cp_register(pcl, abrl);
	cp_register(pch, abrh);
	// load adr high from memory
	set_rw2read();
	access_memory();
	// add X reg to adr low from adr bus
	alu(ALU_OP_ADD, dbr, idx, abrl, localflags);
	// write zp to adr high
	cp_register(zero, abrh);
	// load adr low from memory
	set_rw2read();
	access_memory();
	...
	cp_register(dbr, low);
	// add 1 to adr low
	alu(ALU_OP_ADD_WITH_CARRY, abrl, one, abrl, localflags);
	// zp adr high
	cp_register(zero, abrh);
	// load adr high from memory
	set_rw2read();
	access_memory();
	// write adr low and high to adr bus
...
void adr_izy(){
	...
	access_memory();
	// add X reg to adr low from adr bus
	alu(ALU_OP_ADD, dbr, idy, abrl, localflags);
	// write zp to adr high
	cp_register(zero, abrh);
	// load adr low from memory
	set_rw2read();
